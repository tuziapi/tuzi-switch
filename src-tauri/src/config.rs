...
///   且不一定等于用户目录，可能导致 `.tuzi-switch/tuzi-switch.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
pub fn get_app_config_dir() -> PathBuf {
    ...
    let default_dir = crate::product::default_app_config_dir();

    // 兼容 v3.10.3：当用户环境存在 `HOME` 且与真实用户目录不同，
    // v3.10.3 可能在 `HOME/.tuzi-switch/` 下创建/使用了数据库。
    // 这里仅在“默认位置没有数据库”时回退到旧位置，避免再次出现“供应商消失”问题，
    // 同时也避免新安装因为 `HOME` 被设置而写入非预期路径。
    #[cfg(windows)]
    {
        let default_db = default_dir.join(crate::product::DATABASE_FILE_NAME);
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                let trimmed = home_env.trim();
                if !trimmed.is_empty() {
                    let legacy_dir =
                        PathBuf::from(trimmed).join(crate::product::APP_CONFIG_DIR_NAME);
                    if legacy_dir.join(crate::product::DATABASE_FILE_NAME).exists() {
                        log::info!(
