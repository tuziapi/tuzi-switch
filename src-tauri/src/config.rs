...
pub fn get_app_config_dir() -> PathBuf {
    ...
    let default_dir = get_home_dir().join(".tuzi-switch");
    ...
    #[cfg(windows)]
    {
        let default_db = default_dir.join("tuzi-switch.db");
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                let trimmed = home_env.trim();
                if !trimmed.is_empty() {
                    let legacy_dir = PathBuf::from(trimmed).join(".tuzi-switch");
                    if legacy_dir.join("tuzi-switch.db").exists() {
                        log::info!(
