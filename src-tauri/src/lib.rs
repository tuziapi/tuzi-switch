...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.tuzi-switch/crash.log）
    ...
    let builder = web_hot_update::register_protocol(builder)
...

            {
                ...
                tauri::async_runtime::spawn_blocking(move || {
                    match crate::codex_history_migration::migrate_codex_defined_state_history_to_unified_bucket(
                        ...
                        Err(e) => {
                            log::warn!("✗ Codex defined state history migration failed: {e}");
                        }
                    }
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first tuziswitch:// URL
                        }
                    }
                }
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::clear_provider_live_config,
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 tuziswitch://...）
