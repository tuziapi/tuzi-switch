//! Panic Hook 模块
//!
//! 在应用崩溃时捕获 panic 信息并记录到 `<app_config_dir>/crash.log` 文件中（默认 `~/.tuzi-switch/crash.log`）。
//! 便于用户和开发者诊断闪退问题。
...
fn default_app_config_dir() -> PathBuf {
    dirs::home_dir()
        ...
        .join(crate::product::APP_CONFIG_DIR_NAME)
}
...
mod tests {
    ...
    fn test_crash_log_path() {
        ...
        assert!(path.to_string_lossy().contains(".tuzi-switch"));
    }
