...
/// Open a trusted business query page and pass the API key as a query
/// parameter. The key is never injected into arbitrary page JavaScript.
#[tauri::command]
pub async fn open_webview_with_key(app: AppHandle, url: String, key: String) -> Result<(), String> {
    ...
    if parsed.scheme() != "https" {
        return Err("仅允许 HTTPS 查询地址".to_string());
    }
    let host = parsed.host_str().unwrap_or_default();
    if !matches!(
        host,
        "check.sydney-ai.com" | "api.tu-zi.com" | "coding.tu-zi.com"
    ) {
        return Err("不受信任的查询地址".to_string());
    }
    let key = key.trim();
    if !key.is_empty() {
...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(crate::product::LATEST_RELEASE_URL, None::<String>)
