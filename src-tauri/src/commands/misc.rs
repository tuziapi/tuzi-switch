...
pub(crate) fn launch_terminal_running(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let (script_file, script_content) = {
        let file = temp_dir.join(format!("tuzi_switch_{}_{}.sh", label, pid));
        let content = format!(
            r#"#!/bin/bash
trap 'rm -f "{script_path}"' EXIT
echo "[tuzi-switch] Starting: {cmd}"
echo ""
{cmd}
echo ""
echo "[tuzi-switch] Command exited. Press any key to close."
read -n 1 -s
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let bat_file = temp_dir.join(format!("tuzi_switch_{}_{}.bat", label, pid));
        let content = format!(
            "@echo off\r\necho [tuzi-switch] Starting: {cmd}\r\necho.\r\n{cmd}\r\necho.\r\necho [tuzi-switch] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            cmd = command_line,
        );
...
mod tests {
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("tuzi-switch-missing-{unique}"));
    ...
    fn iterm2_applescript_cold_start_avoids_current_window_before_one_exists() {
        let script = build_macos_iterm2_applescript(Path::new("/tmp/tuzi_switch_launcher.sh"));
