...
pub(crate) use live::{
    ...
    provider_exists_in_live_config, restore_live_settings_for_provider_backfill,
...
mod tests {
    ...
    #[test]
    #[serial]
    fn reapply_official_codex_history_bucket_is_reversible_without_polluting_template() {
        ...
        db.set_current_provider(AppType::Codex.as_str(), &official.id)
        ...
        crate::settings::update_settings(settings).expect("enable unified history");
        assert!(reapply_current_codex_official_live(&state).expect("write unified live"));

        let live_path = crate::codex_config::get_codex_config_path();
        let live = fs::read_to_string(&live_path).expect("read unified live config");
        let live_doc = live
            .parse::<toml_edit::DocumentMut>()
            .expect("parse live config");
        assert_eq!(live_doc["model_provider"].as_str(), Some("tuziswitch"));
        ...
        assert!(stored_config.contains("model_provider = \"codex\""));
        assert!(!stored_config.contains("tuziswitch"));

        let mut settings = crate::settings::get_settings();
        settings.unify_codex_session_history = false;
        crate::settings::update_settings(settings).expect("disable unified history");
        assert!(reapply_current_codex_official_live(&state).expect("restore exact live"));

        let restored = fs::read_to_string(&live_path).expect("read restored live config");
        let restored_doc = restored
            .parse::<toml_edit::DocumentMut>()
            .expect("parse restored live config");
        assert_eq!(restored_doc["model_provider"].as_str(), Some("codex"));
        assert!(!restored.contains("tuziswitch"));

        let mut settings = crate::settings::get_settings();
        settings.unify_codex_session_history = true;
        crate::settings::update_settings(settings).expect("re-enable unified history");
        futures::executor::block_on(
            state
                .proxy_service
                .update_live_backup_from_provider(AppType::Codex.as_str(), &official),
        )
        .expect("write unified restore backup");
        let unified_backup =
            futures::executor::block_on(db.get_live_backup(AppType::Codex.as_str()))
                .expect("read unified backup")
                .expect("unified backup exists");
        assert!(unified_backup.original_config.contains("tuziswitch"));

        let mut settings = crate::settings::get_settings();
        settings.unify_codex_session_history = false;
        crate::settings::update_settings(settings).expect("disable unified history for backup");
        assert!(reapply_current_codex_official_live(&state).expect("restore exact backup"));
        let exact_backup = futures::executor::block_on(db.get_live_backup(AppType::Codex.as_str()))
            .expect("read exact backup")
            .expect("exact backup exists");
        let exact_backup: Value =
            serde_json::from_str(&exact_backup.original_config).expect("parse exact backup");
        let exact_config = exact_backup["config"].as_str().expect("exact config text");
        let exact_doc = exact_config
            .parse::<toml_edit::DocumentMut>()
            .expect("parse exact backup config");
        assert_eq!(exact_doc["model_provider"].as_str(), Some("codex"));
        assert!(!exact_config.contains("tuziswitch"));

        let stored = db
            .get_provider_by_id(&official.id, AppType::Codex.as_str())
            .expect("read final stored provider")
            .expect("final stored provider exists");
        assert!(!stored.settings_config.to_string().contains("tuziswitch"));

        drop(state);
        drop(home);
        crate::settings::reload_settings().expect("restore process settings");
    }
