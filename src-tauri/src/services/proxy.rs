...
impl ProxyService {
    ...
    /// Update the restore backup from the provider template without preserving
    /// the current Codex history bucket. Used when unified history is disabled.
    ...
    async fn update_live_backup_from_provider_inner(
        ...
        app_type: &str,
        provider: &Provider,
        apply_unified_history_bucket: bool,
    ) -> Result<(), String> {
        let app_type_enum =
        ...
        if matches!(app_type_enum, AppType::Codex) {
            ...
            if apply_unified_history_bucket {
...
"#
    ...
    async fn codex_backup_rebuild_uses_live_auth_when_backup_is_missing() {
        ...
        service
            .update_live_backup_from_provider_inner("codex", &official, true)
            .await
