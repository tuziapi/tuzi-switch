...
pub const CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "tuzi-switch-model-catalog.json";
...
fn is_managed_codex_model_catalog_filename(filename: &str) -> bool {
    filename == CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME
        || filename == LEGACY_CC_SWITCH_CODEX_MODEL_CATALOG_FILENAME
}
...
fn managed_env_path() -> PathBuf {
    get_app_config_dir().join("codex-env")
}

fn read_managed_env_key(env_key: &str) -> Option<String> {
    if !is_valid_env_key_name(env_key) {
        return None;
    }
    std::env::var(env_key)
        ...
        .or_else(|| {
            let content = fs::read_to_string(managed_env_path()).ok()?;
            content.lines().find_map(|line| {
                let (key, value) = line.split_once('=')?;
                (key == env_key).then(|| value.to_string())
            })
        })
}

fn write_managed_env_key(env_key: &str, value: &str) -> Result<(), AppError> {
    validate_env_key_name(env_key)?;
    if value.contains(['\r', '\n']) {
        return Err(AppError::Message(
            "Codex API key contains an invalid line break".to_string(),
        ));
    }
    let path = managed_env_path();
            ...
            let should_remove = doc
                ...
                .map(|path| {
                    Path::new(path)
                        ...
                        .map(is_managed_codex_model_catalog_filename)
                        .unwrap_or(false)
                })
                .unwrap_or(false);
            if should_remove {
...
/// Write a provider template without reusing or injecting a unified history bucket.
/// Used only while disabling unified history so the provider's own
/// `model_provider` becomes authoritative again.
...
fn active_codex_env_key_state(config_text: &str) -> Result<Option<Option<String>>, AppError> {
    if config_text.trim().is_empty() {
        return Ok(None);
    }
    let doc = config_text
...
fn extract_codex_env_key(config_text: &str) -> Option<String> {
    active_codex_env_key_state(config_text)
        .ok()
        .flatten()
        .flatten()
}

fn set_active_codex_env_key(config_text: &str, env_key: Option<&str>) -> Result<String, AppError> {
    if config_text.trim().is_empty() {
    ...
    if let Some(key) = env_key.map(str::trim).filter(|key| !key.is_empty()) {
        ...
        {
            return Err(AppError::Message(format!("Invalid Codex env_key: {key}")));
        }
        provider_table["env_key"] = toml_edit::value(key);
    } else {
...
pub fn prepare_codex_provider_live_config(
    ...
    let token = extract_codex_auth_api_key(auth)
        .or_else(|| extract_codex_env_key(config_text).and_then(|key| read_managed_env_key(&key)))
...
pub fn restore_codex_provider_token_for_backfill(
    ...
    if let Some(obj) = settings.as_object_mut() {
        if let Some(env_key) = template_settings
            .get("config")
            ...
            })
        {
            ...
            obj.insert(
                "config".to_string(),
                Value::String(remove_codex_experimental_bearer_token(&config_text)?),
            );
...
fn restore_codex_route_for_backfill(
    settings: &mut Value,
    template_settings: &Value,
) -> Result<(), AppError> {
    let Some(live_config) = settings
        .get("config")
        .and_then(Value::as_str)
        .map(str::to_string)
    else {
    ...
    let Some(template_config) = template_settings.get("config").and_then(Value::as_str) else {
        return Ok(());
    };
    let template_env_key = active_codex_env_key_state(template_config)?;
    let Some(template_env_key) = template_env_key else {
        return Ok(());
    };

    let live_env_key = active_codex_env_key_state(&live_config)?.flatten();
    let mut restored = set_active_codex_env_key(&live_config, template_env_key.as_deref())?;
...
pub fn restore_codex_settings_for_backfill(
    ...
    restore_codex_route_for_backfill(settings, template_settings)?;
...
"#;
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        ...
        assert!(injected.contains("model_provider = \"tuziswitch\""));
...
"#;
    ...
    fn unified_session_bucket_skips_conflicting_custom_table() {
        // 旧 CC 的 custom 表与 Tuzi 的稳定桶不同名，不应阻止注入；原表必须保留。
        let stale = r#"[model_providers.custom]
name = "Relay"
base_url = "https://relay.example/v1"
"#;
        let migrated = inject_codex_unified_session_bucket(stale).expect("inject");
        assert!(migrated.contains("model_provider = \"tuziswitch\""));
        assert!(migrated.contains("[model_providers.custom]"));
        assert!(migrated.contains("[model_providers.tuziswitch]"));

        // 已是注入形态的 Tuzi 表（如重复注入）应保持幂等。
...
"#;
    ...
    #[test]
    #[serial]
    fn third_party_switch_preserves_official_auth_when_enabled() {
        with_test_home(|| {
            ...
            .expect("enable preserve official auth");
            ...
            fs::write(
                &auth_path,
                r#"{"auth_mode":"chatgpt","OPENAI_API_KEY":"official-key"}"#,
            )
            .expect("seed official auth");
...
"#,
            ...
            let auth_text = fs::read_to_string(&auth_path).expect("read auth");
            assert!(auth_text.contains("official-key"));
            assert!(!auth_text.contains("third-party-key"));
            let config_text = read_codex_config_text().expect("read config");
            assert!(config_text.contains("model_provider = \"rightcode\""));
        });
    }

    #[test]
    #[serial]
    fn third_party_switch_writes_auth_when_preservation_is_disabled() {
        with_test_home(|| {
            crate::settings::reload_settings().expect("reload settings");
            crate::settings::update_settings(crate::settings::AppSettings {
            ...
            .expect("disable preserve official auth");

            let codex_dir = get_codex_config_dir();
            fs::create_dir_all(&codex_dir).expect("create codex dir");
            let auth_path = get_codex_auth_path();
            fs::write(&auth_path, r#"{"OPENAI_API_KEY":"official-key"}"#)
                .expect("seed official auth");
    ...
    #[test]
    #[serial]
    fn official_switch_only_writes_auth_with_login_material() {
        with_test_home(|| {
            crate::settings::reload_settings().expect("reload settings");
            ...
            fs::create_dir_all(&codex_dir).expect("create codex dir");
            let auth_path = get_codex_auth_path();
            fs::write(&auth_path, r#"{"OPENAI_API_KEY":"existing-key"}"#)
                .expect("seed existing auth");

            write_codex_live_for_provider(Some("official"), &json!({"auth_mode": "chatgpt"}), None)
                .expect("ignore incomplete official auth");
            let unchanged = fs::read_to_string(&auth_path).expect("read unchanged auth");
            assert!(unchanged.contains("existing-key"));

            write_codex_live_for_provider(
                Some("official"),
                &json!({"auth_mode": "chatgpt", "tokens": {"access_token": "official-token"}}),
                None,
            )
            .expect("write complete official auth");
            let updated = fs::read_to_string(&auth_path).expect("read updated auth");
            assert!(updated.contains("official-token"));
            assert!(!updated.contains("existing-key"));
        });
    }
