...
impl Default for ChatToResponsesState {
    fn default() -> Self {
        Self {
            ...
            completed: false,
            response_id: "resp_tuziswitch".to_string(),
            model: String::new(),
            created_at: 0,
            next_output_index: 0,
            text: ItemState::default(),
...
impl ChatToResponsesState {
    fn handle_chat_chunk(&mut self, chunk: &Value) -> Vec<Bytes> {
        ...
        if let Some(created) = chunk.get("created").and_then(|value| value.as_u64()) {
        ...
        if let Some(delta) = choice.get("delta") {
            ...
            if let Some(tool_calls) = delta.get("tool_calls").and_then(|value| value.as_array()) {
                ...
                for tool_call in tool_calls {
                    events.extend(
                        self.push_tool_call_delta(tool_call, reasoning_for_tool.as_deref()),
                    );
                }
            }
        }
    ...
    fn ensure_response_started(&mut self) -> Vec<Bytes> {
        ...
        vec![
            sse_event(
                "response.created",
                json!({"type": "response.created", "response": self.base_response("in_progress", Vec::new())}),
            ),
            sse_event(
                "response.in_progress",
                json!({"type": "response.in_progress", "response": self.base_response("in_progress", Vec::new())}),
            ),
        ]
    }
    ...
    fn push_tool_call_delta(&mut self, tool_call: &Value, reasoning: Option<&str>) -> Vec<Bytes> {
        ...
        if should_add {
            ...
            events.push(sse_event(
                "response.output_item.added",
                json!({"type": "response.output_item.added", "output_index": assigned, "item": item}),
            ));
