...
#[test]
fn provider_service_clear_claude_config_preserves_malformed_live_settings() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    ensure_test_home();
    ...
    std::fs::write(&settings_path, malformed).expect("seed malformed claude settings");
    ...
    assert!(ProviderService::clear_live_config(&state, AppType::Claude, "claude-tuzi").is_err());
    assert_eq!(
        std::fs::read_to_string(&settings_path).expect("read settings"),
        malformed,
        "a failed parse must never overwrite the user's live settings"
    );
}
...
#[test]
fn provider_service_clear_gemini_config_preserves_malformed_live_settings() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let malformed = "{ invalid json";
    std::fs::write(&settings_path, malformed).expect("seed malformed gemini settings");
    ...
    assert!(ProviderService::clear_live_config(&state, AppType::Gemini, "gemini-tuzi").is_err());
    assert_eq!(
        std::fs::read_to_string(&settings_path).expect("read settings"),
        malformed,
        "a failed parse must never overwrite the user's live settings"
