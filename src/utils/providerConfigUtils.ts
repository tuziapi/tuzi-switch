...
/** Update the credential environment variable used by the active Codex route. */
export const setCodexEnvKey = (configText: string, envKey: string): string => {
  const normalizedText = normalizeTomlText(configText);
  const trimmed = envKey.trim();
  if (!normalizedText || !trimmed) return configText;
  ...
  const providerLineIndex = getTopLevelModelProviderLineIndex(lines);
  lines.splice(
    providerLineIndex !== -1 ? providerLineIndex + 1 : topLevelEndIndex,
    0,
    `env_key = "${escaped}"`,
  );
  return finalizeTomlText(lines);
...
export const generateCodexCopyEnvKey = (
  ...
  const prefix =
    providerName
      ...
      .replace(/_+/g, "_")
      .toUpperCase() || "CUSTOM";
  const base = `${prefix}_CODEX_API_KEY`;
  const taken = new Set(
...
/** Duplicated providers must not share either persisted keys or env_key names. */
export const rewriteDuplicatedCodexSettingsConfig = (
  ...
  if (typeof nextSettingsConfig.config === "string") {
    nextSettingsConfig.config = updateCodexExperimentalBearerToken(
      setCodexEnvKey(nextSettingsConfig.config, nextEnvKey),
      "",
    );
  }
...
/** Resolve env_key from the active route, legacy top-level config or profile. */
export const getCodexEnvKey = (configText: string): string | null => {
  if (!configText) return null;
  const lines = configText.split("\n");
  const findSectionEnvKey = (sectionHeader: string) => {
    let inSection = false;
    for (const line of lines) {
      ...
      const match = line.match(TOML_ENV_KEY_PATTERN);
  ...
  const activeProvider = lines
    .map((line) => line.match(TOML_MODEL_PROVIDER_LINE_PATTERN)?.[2])
    .find(Boolean);
  if (activeProvider) {
    const key = findSectionEnvKey(`[model_providers.${activeProvider}]`);
    if (key) return key;
  }

  for (let index = 0; index < getTopLevelEndIndex(lines); index += 1) {
    const match = lines[index].match(TOML_ENV_KEY_PATTERN);
    if (match?.[2]) return match[2];
  }
  ...
  const profile = lines
    .map(
      (line) =>
        line.match(/^\s*profile\s*=\s*(["'])([^"'\r\n]+)\1\s*(?:#.*)?$/)?.[2],
    )
    .find(Boolean);
  return profile ? findSectionEnvKey(`[profiles.${profile}]`) : null;
};
...
export const getCodexProviderEnvKeyFromSettings = (
  ...
  const configText = typeof value.config === "string" ? value.config : "";
  const tomlEnvKey = getCodexEnvKey(configText)?.trim();
  if (tomlEnvKey) return tomlEnvKey;

  const env = value.env;
  if (!env || typeof env !== "object") return "";
  const envKey = (env as Record<string, unknown>).envKey;
