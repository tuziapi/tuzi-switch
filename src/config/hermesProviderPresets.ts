...
import {
  TUZI_CLAUDE_AGENT_ROUTES,
  TUZI_CODEX_AGENT_ROUTES,
...
const tuziHermesPresets: HermesProviderPreset[] = [
  ...TUZI_CODEX_AGENT_ROUTES.map((route) => ({
    name: route.name,
    websiteUrl: "",
    ...
    icon: route.icon,
    suggestedDefaults: {
      model: { default: route.model, provider: route.name },
    },
  })),
  ...TUZI_CLAUDE_AGENT_ROUTES.map((route) => ({
    name: route.name,
    websiteUrl: "",
    apiKeyUrl: route.apiKeyUrl,
    settingsConfig: {
      ...
      base_url: route.baseUrl,
      api_key: "",
      api_mode: "anthropic_messages" as const,
      models: [
        {
          id: route.model,
          name: route.modelName,
