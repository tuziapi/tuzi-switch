...
/**
 * Build a Tuzi-managed Codex route without storing the credential in auth.json.
 * The provider-scoped env key keeps multiple business routes isolated while
 * the non-empty actor header enables Codex's local image generation tool.
 */
function generateTuziManagedConfig(
  providerName: string,
  baseUrl: string,
  envKey: string,
  modelName = TUZI_CODEX_MODEL,
): string {
  const id =
    providerName
      .toLowerCase()
      .replace(/[^a-z0-9_]/g, "_")
      .replace(/^_+|_+$/g, "") || "tuziswitch";

  return `model_provider = "${id}"
model = "${modelName}"
model_reasoning_effort = "high"
disable_response_storage = true
...
const tuziCodexPresets: CodexProviderPreset[] = TUZI_CODEX_ROUTES.map(
  (route) => ({
    name: route.name,
    websiteUrl: "",
    apiKeyUrl: route.apiKeyUrl,
    auth: {},
    config: generateTuziManagedConfig(
      route.providerId,
      route.baseUrl,
      route.envKey,
    ),
    category: "aggregator",
    endpointCandidates: [...route.endpointCandidates],
    icon: route.icon,
  }),
);
