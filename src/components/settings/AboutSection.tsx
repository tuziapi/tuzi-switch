...
export function AboutSection({ isPortable }: AboutSectionProps) {
  ...
  const handleCheckUpdate = useCallback(async () => {
    if (hasUpdate) {
      if (isPortable || updateInfo?.manual) {
        try {
          ...
          <div className="flex items-center gap-8">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant={
                    webUpdateStatus?.usingHotAssets ? "secondary" : "outline"
                  }
                  className="gap-1.5 bg-background/80"
                >
                  <span className="text-muted-foreground">
                    {t("settings.webHotUpdate")}
                  </span>
                  <span className="font-medium">
                    {webUpdateStatus?.usingHotAssets
                      ? t("settings.webHotUpdateEnabled")
                      : t("settings.webHotUpdateBundled")}
                  </span>
                </Badge>
              </div>
            </div>
            ...
            <Button
              ...
              onClick={() =>
                settingsApi.openExternal(
                  "https://github.com/tuziapi/tuzi-switch",
                )
              }
