...
export function ProviderPresetSelector({
  ...
  const getPresetTooltip = (preset: AnyPreset, category: string) => {
    if (preset.endpointCandidates?.[0]) {
      return preset.endpointCandidates[0];
    }

    if ("baseURL" in preset && typeof preset.baseURL === "string") {
    ...
    if ("settingsConfig" in preset) {
      const settingsConfig = preset.settingsConfig as {
        env?: Record<string, unknown>;
      };
      const env = settingsConfig.env;
      const url =
        env?.ANTHROPIC_BASE_URL || env?.GOOGLE_GEMINI_BASE_URL || undefined;
          ...
          return entries.map((entry) => {
            ...
            return (
              <button
                ...
                title={getPresetTooltip(entry.preset, category)}
              >
