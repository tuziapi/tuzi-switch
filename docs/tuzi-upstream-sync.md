...
迁移/还原测试、项目切换能力检查、热更新契约检查与全套 CI 共同保护。任一门禁失败时都不会
合并、打 tag 或发布。
...
| 界面行为       | 主界面、通用设置和托盘必须保留项目切换；Web 热更新包必须包含项目切换及 Tuzi 功能契约             |
...
> 项目切换是 Tuzi 保留能力：主界面可选择“不使用项目”，通用设置可控制入口显示，托盘可按分组切换；后续 CC 同步和 Web 热更新不得移除。
