...
### 面向 Claude Code、Codex 与 OpenClaw 的兔子业务改版入口
...
tuzi-switch 是基于 CC Switch 定制的兔子业务版本。它不再只是一个通用的多工具管理器，而是围绕 Claude Code、Codex、OpenClaw 的兔子接入流程做了定制化改版。

当前版本重点解决的是兔子客户接入效率问题：用户只需要输入一次 key，即可更快完成业务线路接入，而不必自己手动修改配置文件。

- **兔子优先接入** — 为 Claude Code、Codex、OpenClaw 提供更直观的快速接入入口
- **业务线路配置更短路径** — 用户输入 key 后即可完成接入配置
- **品牌化改版界面** — 增加兔子图标、业务入口与产品化视觉调整
