...
- P0 已完成：Claude、Codex、Gemini、OpenClaw 四个入口的第一轮兔子路线管理改版已经落地，基础卡片结构、模块色语义、当前线路展示方式已经统一
- P0 已完成：Codex 已拆分为兔子主线路与 Coding 特别线路，一键配置和手动配置之间的语义差异已明显缩小
- P0 已完成：Gemini 业务接入首版已完成，当前支持“原版 Gemini + 兔子 API”与“gac 改版 Gemini”两种方案
- P0 已完成：四个入口的状态卡、刷新状态、图标缺失、当前线路误判等问题已完成一轮修正
- P1 进行中：继续精修四个入口的状态读取链路，重点解决“已配置但状态不可信”“读取卡住”“当前线路与实际不一致”这类体验问题
- P1 进行中：继续统一四个入口在深色 / 浅色主题下的视觉层级，包括路线卡、状态 badge、输入区、提示区、列表卡高亮与空状态表达
- P1 进行中：继续优化 OpenClaw 的业务接入体验，把 Tuzi / GAC / Claude 方向 / Codex 方向之间的选择关系讲得更清楚，同时减少重复说明
- P1 待做：重新梳理会话管理能力，明确 OpenClaw 会话恢复、会话边界与可恢复范围，必要时补齐恢复方案
- P1 待做：补充更完整的 release 说明、升级提醒、安装提示与客户视角文案，让每次发版都更容易对外说明
- P2 规划中：在拿到稳定鉴权方案后，对接兔子后端聚合接口，补齐 summary / trend / distribution 等真实业务数据视图
- P2 规划中：继续优化安装说明与 release 流程，解决一键安装脚本、`prerelease` 策略与“安装到最新版”之间的不一致
- P2 规划中：持续减少文档、界面与兼容路径中残留的上游命名，逐步统一为兔子业务版本表达
- P2 规划中：继续整理各入口的一键配置写入结果与 provider 数据结构，减少前端判断分支和特殊兼容逻辑
- 补充提醒：后续需要和老板确认，是否可以提供可接入的数据接口或聚合口径，方便后面接真实业务数据
