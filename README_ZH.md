...
> 兔子switch 以 [CC Switch](https://github.com/farion1231/cc-switch) 为完整底座，在保留上游功能的基础上增加 Tuzi Provider、统一会话、数据迁移与独立发布能力。
...
**兔子switch** 为你提供一个桌面应用来管理所有支持的 AI 工具。无需手动编辑配置文件，你将获得一个可视化界面，一键将供应商导入应用，一键在不同的供应商之间进行切换，内置 50+ 供应商预设、统一的 MCP, SKILLS 管理以及系统托盘即时切换功能——所有操作都基于可靠的 SQLite 数据库和原子写入机制，保护你的配置不被损坏。
...
- **Deep Link** (`tuziswitch://`) — 通过 URL 一键导入供应商、MCP 服务器、提示词和技能
- 深色 / 浅色 / 跟随系统主题、开机自启、自动更新、原子写入、自动备份、国际化（简中/繁中/英/日）

## 常见问题

<details>
<summary><strong>兔子switch 支持哪些 AI 工具？</strong></summary>

兔子switch 支持七个工具：**Claude Code**、**Claude Desktop**、**Codex**、**Gemini CLI**、**OpenCode**、**OpenClaw** 和 **Hermes**。每个工具都有专属的供应商预设和配置管理。
...
兔子switch 使用“通用配置片段”功能，在不同的供应商之间传递 Key 和请求地址之外的通用数据，您可以在“编辑供应商”菜单的“通用配置面板”里，点击“从当前供应商提取”，把所有的通用数据提取到通用配置中，之后在新建“供应商”的时候，只要勾选“应用通用配置”（默认勾选），就会把插件等数据写入到新的供应商配置中。您的所有配置项都会保存在运行本软件的时候，第一次导入的默认供应商里面，不会丢失。
...
兔子switch macOS 版本已通过 Apple 代码签名和公证，可直接下载安装，无需额外操作。推荐使用 `.dmg` 安装包。
...
- **数据库**：`~/.tuzi-switch/tuzi-switch.db`（SQLite — 供应商、MCP、提示词、技能）
- **本地设置**：`~/.tuzi-switch/settings.json`（设备级 UI 偏好设置）
- **备份**：`~/.tuzi-switch/backups/`（自动轮换，保留最近 10 个）
- **SKILLS**：`~/.tuzi-switch/skills/`（默认通过软链接连接到对应应用）
- **技能备份**：`~/.tuzi-switch/skill-backups/`（卸载前自动创建，保留最近 20 个）
...
如果你是从桌面图标启动的，请把它写进 `.desktop` 的 `Exec=` 行（如 `env TUZI_SWITCH_GDK_BACKEND=wayland /path/to/AppImage`），或在会话环境中设置。该变量是通用的：在 tiling Wayland 合成器（sway/Hyprland）下若出现点击失效，可反过来设 `TUZI_SWITCH_GDK_BACKEND=x11`。不设置则保持默认行为。
...
从 [Tuzi Releases](https://github.com/tuziapi/tuzi-switch/releases) 下载
`tuzi-switch-windows-x86_64.msi` / `tuzi-switch-windows-aarch64.msi`，或对应的 portable 压缩包。
...
> **注意**：正式 Release 流程强制执行 Apple 代码签名和公证。

### Linux 用户

从 [Tuzi Releases](https://github.com/tuziapi/tuzi-switch/releases) 下载最新 Linux 安装包：
...
- **SSOT**（单一事实源）：所有数据存储在 `~/.tuzi-switch/tuzi-switch.db`（SQLite）
...
正式版默认开启最小化匿名使用统计，用户可在设置中随时关闭。统计只使用固定枚举记录应用、供应商、代理、认证、同步、更新和设置操作次数，不收集 API Key、账号标识、供应商名称或地址、对话、请求内容、文件路径、配置正文和错误详情。

统计请求由应用直接发送至 `umami.tu-zi.com`。部署方应在 Umami 和反向代理中配置合理的 IP 最小化、日志脱敏和数据保留策略。
